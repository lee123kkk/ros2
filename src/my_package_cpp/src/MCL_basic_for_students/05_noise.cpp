...
int main()
{
    ...
    // 1. 노이즈 설정: 직진(5.0), 회전(0.1), 센서(5.0)
    // 여기서 Sense Noise(5.0)가 센서 측정값의 '표준편차(Sigma)' 역할을 합니다.
    myrobot.set_noise(5.0, 0.1, 5.0);
    
    // 2. 로봇의 초기 위치 고정 (x=30.0, y=50.0, orient=90도)
    myrobot.set(30.0, 50.0, M_PI / 2.0);
    
    // 로봇을 움직이지 않고(move 함수 주석 처리 또는 삭제), 센서만 5회 연속 측정
    cout << "=== 고정된 위치(30.0, 50.0)에서 센서 5회 측정 ===" << endl;
    for (int i = 0; i < 5; i++) {
        cout << "[" << i + 1 << "회차] " << myrobot.read_sensors() << endl;
    }
