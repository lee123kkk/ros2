...
int main()
{
    ...
    double beta = 0.0;
    double mw = max(w, n); // 전체 입자 중 가장 높은 가중치 (Wmax)

    cout << "=== 리샘플링 휠(Resampling Wheel) 알고리즘 분석 ===" << endl;
    cout << "초기 설정된 입자 개수(n): " << n << "개" << endl;
    cout << "최대 가중치(Wmax): " << mw << endl;

    // 선택된 인덱스의 횟수를 저장할 배열 (경향성 파악용)
    vector<int> index_counts(n, 0);

    for (int i = 0; i < n; i++) {
        // beta에 0.0 ~ 2*Wmax 사이의 난수를 더함
        beta += gen_real_random() * 2.0 * mw;
        
        // beta가 현재 가중치보다 크면 다음 인덱스로 이동하며 beta 감소
        while (beta > w[index]) {
            beta -= w[index];
            index = mod((index + 1), n);
        }
    ...
    // 결과 출력 (어떤 입자가 얼마나 살아남았는지)
    cout << "\n[생존한 입자(Particle) 분포 요약]" << endl;
    int survived_count = 0;
    for (int i = 0; i < n; i++) {
        if (index_counts[i] > 0) {
            survived_count++;
            // n이 30 이하일 때만 상세 출력, 그 이상은 너무 길어지므로 생략
            if (n <= 30) {
                cout << "인덱스 [" << i << "]번 입자: " << index_counts[i] << "회 복제됨 (가중치: " << w[i] << ")" << endl;
            }
        }
    }
    cout << "\n-> 결론: 총 " << n << "개의 입자 중 가중치가 높은 " << survived_count << "개의 고유 입자만 살아남아 전체를 대체했습니다." << endl;

    //Evaluate the Error (오차 평가)
    cout << "최종 시스템 오차 Evaluation = " << evaluation(myrobot, p, n) << endl;
